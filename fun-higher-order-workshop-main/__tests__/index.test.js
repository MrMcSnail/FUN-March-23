...
describe('Higher Order Functions', () => {
  ...
  describe('once', () => {
    it('returns a function', () => {
      expect(typeof once(increment)).toEqual('function');
    });
    it('repeated calls to the returned function give the same value as first invocation after initialisation', () => {
      function multiplyBy2(n) {
        return n * 2;
      }
      const by2Once = once(multiplyBy2);
      const six = by2Once(3);
      by2Once();
      const third = by2Once(0);
      expect(third).toBe(six);
    });
  });
